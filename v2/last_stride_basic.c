...
void l2_prefetcher_operate(int cpu_num, unsigned long long int addr, unsigned long long int ip, int cache_hit)
{
 ...
 // printf("(%d) ",(int) (addr-last_addr)); 
  ...
  if(last_addr==-1) last_addr=addr;
	...
	if(stride > -PAGE_SIZE &&stride < PAGE_SIZE && !cache_hit && get_l2_read_queue_occupancy(cpu_num) < 32 && get_l2_mshr_occupancy(cpu_num)<16 ){
 		//printf("(%lli) \n ",stride); 
 		int succ = l2_prefetch_line(cpu_num,addr,addr+stride,FILL_L2);
 		//if(!succ) printf("Prefetch have fail %i\n",succ);
  	}	
 	last_addr = addr;
  }
}
